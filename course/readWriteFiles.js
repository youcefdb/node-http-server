...
import { existsSync } from 'fs'; //check existing(not recommend) (sync fun => no promies)
import {readFile, appendFile, writeFile, unlink, rename, mkdir, rmdir} from 'fs/promises';
...
try{
    ...
    if(!existsSync(path.join(__direName, "newDirc"))){
        await mkdir(path.join(__direName, "newDirc"));
    }
}catch(err){
...
//you have to import "existsSync(path)" from "fs" (because is Sync) to check for directory or file existing (race condition ❌)